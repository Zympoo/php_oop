...
class UsersRepository
{
    ...
    {
        $sql = "SELECT u.id, u.email, u.name, u.is_active, r.name AS
                role_name
                FROM users u
    ...
    {
        $sql = "SELECT u.id, u.email, u.password_hash, u.name, u.is_active,
                r.name AS role_name
                FROM users u
                JOIN roles r ON r.id = u.role_id
                WHERE u.email = :email
    ...
    public function create(string $email, string $name, string
                                  $plainPassword, int $roleId): void
    {
        $sql = "INSERT INTO users (email, name, password_hash, role_id,
                is_active)
    ...
    {
        $sql = "SELECT u.id, u.email, u.name, u.role_id, u.is_active, r.name
                AS role_name
                FROM users u
                JOIN roles r ON r.id = u.role_id
                WHERE u.id = :id
