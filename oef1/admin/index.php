...
if (!Auth::check() && !in_array($uri, $publicRoutes, true)) {
    header('Location: /php/oef1/admin/login');
    exit;
}
...
$router->get('/posts/{id}/delete', function (int $id): void {
    if (!Auth::isAdmin()) {
        header('Location: /php/oef1/admin/posts');
        exit;
    }
...
$router->get('/users/{id}/delete', function (int $id): void {
    (new UsersController(UsersRepository::make()))->deleteConfirm($id);
});
...
$router->get('/users/{id}', function (int $id): void {
    (new UsersController(UsersRepository::make()))->show($id);
});
